...
public class C_CreateChar extends ClientBasePacket {
	...
	public C_CreateChar(byte[] abyte0, ClientThread client) throws Exception {
		...
		if (client.getAccount().countCharacters() >= maxAmount) {
			_log.fine("帳號: " + client.getAccountName() + " 建立超過" + maxAmount
					+ "個角色人物。");
			S_CharCreateStatus s_charcreatestatus1 = new S_CharCreateStatus(
	...
	private static final int[] FEMALE_LIST = new int[] { 1, 48, 37, 1186, 2796,
			6661, 6650 };
	// XXX 各職業出生地改回台版
	private static final int[] LOCX_LIST = new int[] { 32780, 32714, 33043,
			32780, 32925, 32780, 32714 };
	private static final int[] LOCY_LIST = new int[] { 32781, 32877, 32336,
			32781, 32801, 32781, 32877 };
	...
	private static void initNewChar(ClientThread client, L1PcInstance pc)
		...
		for (L1PcInstance allpc : L1World.getInstance().getAllPlayers()) {
			if (allpc != null) {
				allpc.sendPackets(new S_SystemMessage("★玩家【" + pc.getName()
						+ "】誕生了，職業：【" + msg + "】★"));
			}
		}
		pc.setClanRank(0);
