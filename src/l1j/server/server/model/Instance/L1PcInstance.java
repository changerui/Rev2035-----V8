...
public class L1PcInstance extends L1Character {
	...
	public void receiveDamage(L1Character attacker, double damage,
		...
		if (getCurrentHp() > 0 && !isDead()) {
			...
			System.out
					.println("警告：玩家的ＨＰ減少處理有不正當的地方。※或者從最初開始ＨＰ０﹝L1PcInstance﹞。");
			death(attacker);
		}
	}
		...
		for (int i = 0; i < count; i++) {
			...
			if (item != null) {
				...
				if (Config.Dead_Lost_Item) {
					int itemCount = item.getCount(); // TODO 死亡處罰
					getInventory().removeItem(item, itemCount);// TODO 死亡處罰
					sendPackets(new S_SystemMessage("你損失了(" + item.getLogName()
							+ ")"));
				} else {
	...
	private void levelUp(int gap) {
		...
		if (getLevel() >= 51 && getLevel() - 50 > getBonusStats()) {
			if ((getBaseStr() + getBaseDex() + getBaseCon() + getBaseInt()
					+ getBaseWis() + getBaseCha()) < (Config.BONUS_STATS1 * 6)) { // TODO
																					// 調整能力值上限
		...
		// TODO　 超過 13等傳出新手村 By AMD 改case by hot183
		if (getLevel() >= 13) {
			short outmap = getMapId();
			switch (outmap) {
			case 69:
			...
			case 68:
				L1Teleport.teleport(this, 32580, 32931, (short) 0, (byte) 5,
						true); // <-說話之島座標
				break;
			}
		}
	...
	public void sendPackets(String string) {
