...
public class L1PcInventory extends L1Inventory {
		...
		if (itemExist != null && (itemExist.getCount() + count) > MAX_AMOUNT) {
			if (message) {
				getOwner().sendPackets(
						new S_ServerMessage(166, "所持有的金幣", "超過20億。")); // \f1%0が%4%1%3%2
			}
			return AMOUNT_OVER;
		}
	...
	public int hpRegenPerTick() {
		...
		for (Object itemObject : _items) {
			L1ItemInstance item = (L1ItemInstance) itemObject;
			if (item.isEquipped()) {
				hpr += item.getItem().get_addhpr()+ item.getHpr();//TODO 飾品強化卷軸
			}
		}
		return hpr;
	}
	...
	public int mpRegenPerTick() {
		...
		for (Object itemObject : _items) {
			L1ItemInstance item = (L1ItemInstance) itemObject;
			if (item.isEquipped()) {
				mpr += item.getItem().get_addmpr()+ item.getMpr();//TODO 飾品強化卷軸
			}
		}
		return mpr;
	}
