...
import l1j.server.server.serverpackets.S_ItemStatus;//TODO 裝備保護卷軸by missu0524
...
//TODO 解卡點卷軸 use xpatax code 1/2
...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		if (itemId == 40088 || itemId == 40096 || itemId == 140088) {
			s = readS();
				...
				|| itemId == 240107// TODO 裝備保護卷軸 by missu0524
				|| itemId == 49148) { //飾品強化卷軸
			l = readD();
			...
			if (itemId == 40077 || itemId == L1ItemId.SCROLL_OF_ENCHANT_WEAPON
				...
				if (l1iteminstance1 != null) {
					...
					if (l1iteminstance1.getItem().getType2() == 0) {
						...
						pc.sendPackets(new S_ServerMessage(1308,
				...
				if (l1iteminstance.getItem().getType() == 0) { // アロー
					...
					BroadCastToAll("\\fV" + "恭喜玩家【" + pc.getName()
							+ "】在極大的痛苦中感到精神甦醒。");
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 240106) { // TODO 解卡點卷軸　by xpatax
				...
				// レッドポーション、濃縮体力回復剤、象牙の塔の体力回復剤
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
						...
						|| itemId == 49149 // 夏納的變形捲軸(等級30)
						|| itemId == 49150 // 夏納的變形捲軸(等級40)
						|| itemId == 49151 // 夏納的變形捲軸(等級52)
						|| itemId == 49152 // 夏納的變形捲軸(等級55)
						|| itemId == 49153 // 夏納的變形捲軸(等級60)
						|| itemId == 49154 // 夏納的變形捲軸(等級65)
						|| itemId == 49155) { // 夏納的變形捲軸(等級70)）
					...
					/*
					 * TODO 隱藏之谷、歌唱之島地圖改回台版　by 阿傑 if (itemId >= 40373 && itemId <=
					 * 40382 // 地図各種 || itemId >= 40385 && itemId <= 40390) {
					 */
					 ...
					 */else if (itemId == 41209) { // ポピレアの依頼書 TODO 修正隱藏卷無法使用
	...
	private void SpellBook3(L1PcInstance pc, L1ItemInstance l1iteminstance,
		...
		for (int j6 = 87; j6 <= 91; j6++) {
			L1Skills l1skills = SkillsTable.getInstance().getTemplate(j6);
			String s1 = (new StringBuilder()).append("技術書(")
	...
	private void cancelAbsoluteBarrier(L1PcInstance pc) { // アブソルート バリアの解除
		if (pc.hasSkillEffect(ABSOLUTE_BARRIER)) {
			...
			pc.startHpRegenerationByDoll();// TODO 魔法娃娃回血功能 byhot183
		}
	}
	...
	private void useMagicDoll(L1PcInstance pc, int itemId, int itemObjectId) {
		...
		if (isAppear) {
			...
			int npcId = 0;
			int dollType = 0;
			if (itemId == 41248) {// TODO 魔法娃娃：肥肥
				npcId = 80106;
				dollType = L1DollInstance.DOLLTYPE_BUGBEAR;
			} else if (itemId == 41249) {// TODO 魔法娃娃：小思克巴
				npcId = 80107;
				dollType = L1DollInstance.DOLLTYPE_SUCCUBUS;
			} else if (itemId == 41250) {// TODO 魔法娃娃：野狼寶寶
				npcId = 80108;
				dollType = L1DollInstance.DOLLTYPE_WAREWOLF;
			} else if (itemId == 49037) {// TODO 魔法娃娃：長者
				npcId = 80129;
				dollType = L1DollInstance.DOLLTYPE_BUGELDER;
			} else if (itemId == 49038) {// TODO 魔法娃娃：奎斯坦修
				npcId = 80130;
				dollType = L1DollInstance.DOLLTYPE_CRUSTANCE;
			} else if (itemId == 49039) {// TODO 魔法娃娃：石頭高崙
				npcId = 80131;
				dollType = L1DollInstance.DOLLTYPE_STONEGOLEM;
			} else if (itemId == 60035) {// TODO 魔法娃娃：魔法公主 by hot183
				npcId = 81247;
				dollType = L1DollInstance.DOLLTYPE_PRINCESS;
			} else if (itemId == 60036) {// TODO 魔法娃娃：希爾黛絲 by hot183
				npcId = 81248;
				dollType = L1DollInstance.DOLLTYPE_SEADANCER;
			} else if (itemId == 60037) {// TODO 魔法娃娃：蛇女 by ssississi
				npcId = 81249;
			...
			pc.sendPackets(new S_SkillIconGFX(56, 1800)); // 魔法娃娃狀態圖示
			pc.sendPackets(new S_SystemMessage("魔法娃娃會保護你。"));
			pc.sendPackets(new S_OwnCharStatus(pc));
			pc.getInventory().consumeItem(41246, 50); // 魔法結晶體不足。
		} else {
